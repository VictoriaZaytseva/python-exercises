...
your implementation.  However, can you be sure that you've tested
...
game of chess or some similar game. Can you write something like
this for the elevator?  That is, can you write a simulation that
runs the elevator software through every possible combination of
runtime state, checking for potential problems?  Unlike the previous
idea, this involves no randomness--instead every possible state is
eventually explored.

Yet, another testing strategy might involve the idea of using
a solver. If the elevator has been coded with a suitable set of
invariants or constraints, perhaps those invariants can be used
to determine the set of all valid elevator states.  If you had
such a set, perhaps you could test each entry. Here's the idea:
take a valid elevator state and execute every possible event
on it.  Does any event cause the elevator to enter a bad state?
If so, that's a bug!

Your mission, should you choose to accept it, is to come up with
some kind of test strategy for the elevator using one, all, or
some combination of the ideas described here.
'''
